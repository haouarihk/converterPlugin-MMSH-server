...
class CompilersHandler {
    ...
    convert(token, req) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.compileFile(token, nameWT, compileType);
            yield utils_1.deleteFile(uploadpath);
            this.router.newSocketMessage(token, "log", "zipping the folder");
            yield this.zipTheOutputDirectory(name);
            yield utils_1.deleteDirectory(outputDirPath);
            this.router.newSocketMessage(token, "log", "making a request");
            yield this.makeGetReqForTheFile(URLFILE, downloadpath);
        ...
        setInterval(() => {
            fs.readdir(_dir, (err, files) => {
                ...
                for (const file of files) {
                    var stats = fs.statSync(path_1.join(_dir, file));
                    var mtime = stats.mtime;
                    if (Number(new Date()) - Number(new Date(mtime)) >= this.timetoGarbageCleaner * Minute) {
                        fs.unlink(path_1.join(_dir, file), (err) => {
                            if (err)
                                console.error(err);
                        });
                    }
